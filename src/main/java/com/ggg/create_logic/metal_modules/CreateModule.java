...
import com.ggg.create_logic.ModEvents;
...
public class CreateModule extends Metal.MetalModule {
    ...
    public static void runServer(Runnable task, Metal.Script script){
        if (ModEvents.server == null) return;
        if (script.canFreeze()){
            script.freeze();
            ModEvents.server.execute(() -> {
                task.run();
                script.unfreeze();
            });
        } else task.run();
