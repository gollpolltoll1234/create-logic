...
public class CodeEditorWidget extends AbstractWidget {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (ctrl && keyCode == 86) {
            ...
            if (!clipboard.isEmpty()) {
                ...
                text = text.substring(0, cursorPos) + cb.toString() + text.substring(cursorPos);
                cursorPos += cb.toString().length();
                selectionStart = selectionEnd = cursorPos;
                updateScroll();
            }
            return true;
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        if (!isFocused()) return false;
        if (codePoint == '\b' || codePoint == '\u007f' || Character.isISOControl(codePoint)) {
            return true;
        }
        if (selectionStart != selectionEnd) deleteSelection();
